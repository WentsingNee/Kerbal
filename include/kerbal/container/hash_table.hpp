...
namespace kerbal
{
	...
	namespace container
	{
		...
		namespace detail
		{
			...
			template <typename Entity, typename Hash, typename AllocatorBR>
			struct hash_table_allocator_rebind_helper
			{
				private:
			...
			class hash_table_node_allocator_compress_helper :
					private kerbal::utility::member_compress_helper<
								typename hash_table_allocator_rebind_helper<Entity, Hash, NodeAllocatorBR>::ht_node_allocator_type
							>
			{
				protected:
					typedef hash_table_allocator_rebind_helper<Entity, Hash, NodeAllocatorBR>		ht_allocator_rebind_helper;
					typedef typename ht_allocator_rebind_helper::ht_node_allocator_traits	ht_node_allocator_traits;
					typedef typename ht_allocator_rebind_helper::ht_node_allocator_type		ht_node_allocator_type;

				private:
					typedef kerbal::utility::member_compress_helper<ht_node_allocator_type> super;

				protected:
					hash_table_node_allocator_compress_helper() :
							super(kerbal::utility::in_place_t())
					{
					}

					explicit hash_table_node_allocator_compress_helper(const NodeAllocatorBR & alloc) :
							super(kerbal::utility::in_place_t(), alloc)
					{
					}
			...
			template <typename Entity, typename Hash, typename BucketAllocatorBR>
			class hash_table_bucket_allocator_compress_helper :
					private kerbal::utility::member_compress_helper<
									typename hash_table_allocator_rebind_helper<Entity, Hash, BucketAllocatorBR>::bucket_allocator_type
							>
			{
				protected:
					typedef hash_table_allocator_rebind_helper<Entity, Hash, BucketAllocatorBR>		ht_allocator_rebind_helper;
					typedef typename ht_allocator_rebind_helper::bucket_allocator_traits	bucket_allocator_traits;
					typedef typename ht_allocator_rebind_helper::bucket_allocator_type		bucket_allocator_type;

				private:
					typedef kerbal::utility::member_compress_helper<bucket_allocator_type> super;

				protected:
					hash_table_bucket_allocator_compress_helper() :
							super(kerbal::utility::in_place_t())
					{
					}

					explicit hash_table_bucket_allocator_compress_helper(const BucketAllocatorBR & alloc) :
							super(kerbal::utility::in_place_t(), alloc)
					{
					}
...
#		endif
			...
			// Capacity
				...
				{
					return static_cast<size_type>(-1) / sizeof(hash_table_node);
				}
