...
//				k_merge_sort(first, last, cmp);
			...
			KERBAL_CONSTEXPR20
			void list_type_only<T>::k_sort_afford_allocator(ValueTypeAllocator & alloc, const_iterator first, const_iterator last, BinaryPredict cmp)
			{
				if (first == last) {
					return;
				}
				...
				struct cmp_by_ptr_points_to
				{
						...
						KERBAL_CONSTEXPR20
						bool operator()(node_base const * const & lhs, node_base const * const & rhs) const
						{
							node const * a = node::reinterpret_as(lhs);
							node const * b = node::reinterpret_as(rhs);
...
#		endif
			...
			KERBAL_CONSTEXPR20
			void list_type_only<T>::k_sort_afford_allocator(ValueTypeAllocator & alloc, const_iterator first, const_iterator last)
			{
			...
			template <typename ValueTypeAllocator, typename BinaryPredict>
			KERBAL_CONSTEXPR20
			void list_type_only<T>::sort_afford_allocator(ValueTypeAllocator & alloc, BinaryPredict cmp)
			{
				k_sort_afford_allocator(alloc, this->cbegin(), this->cend(), cmp);
			}

			template <typename T>
			template <typename ValueTypeAllocator>
			KERBAL_CONSTEXPR20
			void list_type_only<T>::sort_afford_allocator(ValueTypeAllocator & alloc)
			{
				k_sort_afford_allocator(alloc, this->cbegin(), this->cend());
			}
