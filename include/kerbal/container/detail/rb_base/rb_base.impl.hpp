...
namespace kerbal
{
	...
	namespace container
	{
		...
		namespace detail
		{
			...
			KERBAL_CONSTEXPR14
			inline
			void rb_type_unrelated::k_unhook_node(rb_node_base * n) KERBAL_NOEXCEPT
			{
				if (n == NULL) {
					return;
				}
				if (n->left != get_rb_vnull_node() && n->right != get_rb_vnull_node()) { // two sons
					rb_node_base * replacee = as_rb_node_base(n->right->leftest_offspring(get_rb_vnull_node()));
				...
				if (n->left != get_rb_vnull_node() && n->right == get_rb_vnull_node()) { // only left son
					...
					as_rb_node_base(n->left)->set_black();
					--this->k_cnt;
					return;
				}
				if (n->left == get_rb_vnull_node() && n->right != get_rb_vnull_node()) { // only right son
					...
					as_rb_node_base(n->right)->set_black();
					--this->k_cnt;
					return;
				}
				// leaf
			...
			rb_type_only<T>::rb_normal_impl(Extract & e, KeyCompare & kc, const rb_node_base * pbase, const value_type * & mini, const value_type * & maxi, std::size_t & nodes_cnt, std::size_t & black_cnt)
			{
				...
				const node * pnode = static_cast<const node *>(pbase);
				...
				rb_normal_result_t lresult = rb_normal_impl(e, kc, as_rb_node_base(pnode->left), lmini, lmaxi, nodes_cnt, lblack_cnt);
				...
				rb_normal_result_t rresult = rb_normal_impl(e, kc, as_rb_node_base(pnode->right), rmini, rmaxi, nodes_cnt, rblack_cnt);
				...
				if (rb_node_base::is_black(pnode)) { // p is black
					...
					if (!rb_node_base::is_black(as_rb_node_base(pnode->left))) { // p->left is red
						return RB_NORMAL_RESULT_NEIBOUR_RED;
					}
					if (!rb_node_base::is_black(as_rb_node_base(pnode->right))) { // p->right is red
						return RB_NORMAL_RESULT_NEIBOUR_RED;
					}
				}
			...
			{
				const rb_node_base * root = as_rb_node_base(this->k_head.left);
