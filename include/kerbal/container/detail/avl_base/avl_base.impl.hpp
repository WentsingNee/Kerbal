...
#			endif // if KERBAL_HAS_EXCEPTIONS_SUPPORT
		...
		// lookup
			...
			) const
			{
				...
				if (always_search_left) {
					return this->k_find_hint_search_left(p_start, e, kc, key);
				}
				if (always_search_right) {
					return this->k_find_hint_search_right(p_start, e, kc, key);
				}
				return this->cend();
			}
			...
			{
				return this->k_find_hint_impl(hint, e, kc, key);
			}
			...
			{
				return this->k_find_hint_impl(hint, e, kc, key).cast_to_mutable();
			}
			...
			bool
			avl_type_only<Entity>::
			k_contains_hint(const_iterator hint, Extract & e, KeyCompare & kc, const typename Extract::key_type & key) const
			{
				return this->k_find_hint(hint, e, kc, key) != this->cend();
			}
			...
			>::type
			avl_type_only<Entity>::
			k_contains_hint(const_iterator hint, Extract & e, KeyCompare & kc, const Key & key) const
			{
				return this->k_find_hint(hint, e, kc, key) != this->cend();
			}
...
#		endif
			...
			)
			{
				while (first1 != last1) {
					if (first2 != last2) {
						const typename Extract::key_type & key1 = e(*first1);
						const typename Extract::key_type & key2 = e(*first2);
						if (kc(key1, key2)) { // *first1 < *first2
							first1 = to.k_splice(e, kc, set1, first1);
						} else { // *first1 >= *first2
							if (!static_cast<bool>(kc(key2, key1))) { // *first1 == *first2
								++first1;
							}
							++first2;
						}
			...
			)
			{
				avl_type_only::k_set_difference(
					e, kc, to,
					set1, set1.cbegin(), set1.cend(),
					set2, set2.cbegin(), set2.cend()
				);
			}
			...
			)
			{
				while (first1 != last1) {
					if (first2 != last2) {
						...
						if (kc(key1, key2)) { // *first1 < *first2
							++first1;
						} else if (kc(key2, key1)) { // *first2 < *first1
							++first2;
						} else { // *first1 == *first2
							first1 = to.k_splice(e, kc, set1, first1);
							++first2;
			...
			)
			{
				while (first1 != last1) {
					if (first2 != last2) {
						const typename Extract::key_type & key1 = e(*first1);
						const typename Extract::key_type & key2 = e(*first2);
						if (kc(key2, key1)) { // *first2 < *first1
							first2 = to.k_splice(e, kc, set2, first2);
						} else { // *first1 <= *first2
							bool b = static_cast<bool>(kc(key1, key2));
							first1 = to.k_splice(e, kc, set1, first1);
							if (!b) { // *first1 == *first2
								++first2;
							}
