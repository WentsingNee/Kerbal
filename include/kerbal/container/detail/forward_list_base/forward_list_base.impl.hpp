...
#		endif
			...
			fl_type_only<T>::k_unique_after_using_allocator(NodeAllocator & alloc, const_iterator before_first, const_iterator last, BinaryPredict equal_to)
			{
				size_type cnt = 0;
				if (before_first != last) {
					const_iterator forward(before_first); ++forward;
					while (forward != last) {
						if (equal_to(*before_first, *forward)) {
							++forward;
							k_destroy_node(alloc, fl_type_unrelated::k_unhook_node_after(before_first));
							++cnt;
						} else {
							before_first = forward;
							++forward;
						}
					}
				}
