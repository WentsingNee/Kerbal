...
#			if __cplusplus >= 201103L
				...
				protected:
					...
					template <typename NodeAllocator>
					KERBAL_CONSTEXPR14
					list_allocator_unrelated(NodeAllocator & alloc, NodeAllocator && src_alloc, list_allocator_unrelated && src) KERBAL_CONDITIONAL_NOEXCEPT(
...
#			if __cplusplus >= 201103L
				...
				protected:
					...
					template <typename NodeAllocator>
					KERBAL_CONSTEXPR20
					void assign_using_allocator(NodeAllocator & alloc, NodeAllocator && src_alloc, list_allocator_unrelated && src) KERBAL_CONDITIONAL_NOEXCEPT(
							kerbal::memory::allocator_traits<NodeAllocator>::is_always_equal::value
					);
...
#			else
					...
					template <typename NodeAllocator>
					static iterator emplace_using_allocator(NodeAllocator & alloc, const_iterator pos);

					template <typename NodeAllocator, typename Arg0>
					static iterator emplace_using_allocator(NodeAllocator & alloc, const_iterator pos, const Arg0& arg0);

					template <typename NodeAllocator, typename Arg0, typename Arg1>
					static iterator emplace_using_allocator(NodeAllocator & alloc, const_iterator pos, const Arg0& arg0, const Arg1& arg1);

					template <typename NodeAllocator, typename Arg0, typename Arg1, typename Arg2>
					static iterator emplace_using_allocator(NodeAllocator & alloc, const_iterator pos, const Arg0& arg0, const Arg1& arg1, const Arg2& arg2);
...
#				endif
				...
				protected:
					...
					template <typename BinaryPredict>
					KERBAL_CONSTEXPR20
					static void merge_sort(iterator first, iterator last, BinaryPredict cmp);
				...
				private:
					...
					KERBAL_CONSTEXPR20
					static void radix_sort(iterator first, iterator last, kerbal::type_traits::false_type asc,
										   kerbal::type_traits::integral_constant<std::size_t, RADIX_BIT_WIDTH>) KERBAL_NOEXCEPT;

					template <std::size_t RADIX_BIT_WIDTH>
					KERBAL_CONSTEXPR20
					static void radix_sort(iterator first, iterator last, kerbal::type_traits::true_type desc,
										   kerbal::type_traits::integral_constant<std::size_t, RADIX_BIT_WIDTH>) KERBAL_NOEXCEPT;
				...
				protected:
					...
					KERBAL_CONSTEXPR20
					static void radix_sort(iterator first, iterator last, Order order) KERBAL_NOEXCEPT;

					template <bool is_radix_sort_acceptable_type>
					KERBAL_CONSTEXPR20
					static
					typename kerbal::type_traits::enable_if<is_radix_sort_acceptable_type>::type
					radix_sort(iterator first, iterator last) KERBAL_NOEXCEPT;
				...
				private:
					...
					static
					typename kerbal::type_traits::enable_if<is_radix_sort_acceptable_type>::type
					sort_method_overload(iterator first, iterator last, BinaryPredict cmp);
					...
					static
					typename kerbal::type_traits::enable_if<is_radix_sort_acceptable_type>::type
					sort_method_overload(iterator first, iterator last, std::greater<value_type> cmp);
...
#			if __cplusplus >= 201402L
					...
					static
					typename kerbal::type_traits::enable_if<is_radix_sort_acceptable_type>::type
					sort_method_overload(iterator first, iterator last, std::less<void> cmp);

					template <bool is_radix_sort_acceptable_type>
					KERBAL_CONSTEXPR20
					static
					typename kerbal::type_traits::enable_if<is_radix_sort_acceptable_type>::type
					sort_method_overload(iterator first, iterator last, std::greater<void> cmp);
