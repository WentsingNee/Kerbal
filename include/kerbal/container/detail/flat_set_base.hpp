...
namespace kerbal
{
	...
	namespace container
	{
		...
		namespace detail
		{
			...
			class flat_set_base : public flat_set_common_base<Ordered>
			{
				private:
...
#			else
					...
					template <typename U>
					flat_set_base(const kerbal::assign::assign_list<U> & ilist) :
						super()
					{
					...
					template <typename U>
					flat_set_base(const kerbal::assign::assign_list<U> & ilist, key_compare kc) :
						super(kc)
					{
						this->assign(ilist.cbegin(), ilist.cend());
					}
