...
namespace kerbal
{
	...
	namespace container
	{
		...
		// operation
			...
			void
			sl_type_unrelated::
			k_iter_swap(basic_iterator a, basic_iterator b) KERBAL_NOEXCEPT
			{
			...
			void
			sl_type_unrelated::
			k_reverse(basic_iterator first, basic_iterator last) KERBAL_NOEXCEPT
			{
				if (first == last) {
					return;
				}
...
#		endif
		...
		// private
			...
			sl_type_unrelated::
			k_swap_with_empty(
				sl_type_unrelated & not_empty_list,
				sl_type_unrelated & empty_list
			) KERBAL_NOEXCEPT
			{
			...
			k_swap_type_unrelated(
				sl_type_unrelated & lhs,
				sl_type_unrelated & rhs
			) KERBAL_NOEXCEPT
			{
...
#	if __cplusplus >= 201103L
			...
			void
			sl_type_only<T>::
			k_move_cnstrct_ua_ae(sl_type_only && src) KERBAL_NOEXCEPT
			{
				if (src.empty()) {
					return;
				}
			...
			k_move_cnstrct_ua_helper(
				NodeAllocator & this_alloc,
				NodeAllocator && src_alloc, sl_type_only && src,
...
#	endif
		...
		// assign
			...
			k_cpy_ass_ua_impl(
				NodeAllocator & alloc,
				const NodeAllocator & src_alloc, const sl_type_only & src,
...
#		if __cplusplus >= 201103L
			...
			void
			sl_type_only<T>::
			k_move_assign(NodeAllocator & alloc, sl_type_only && src) KERBAL_NOEXCEPT
			{
				this->k_destroy_using_allocator(alloc);
				this->sl_type_unrelated::k_init_node_base();
				if (!src.empty()) {
			...
			void
			sl_type_only<T>::
			k_mov_ass_ua_ae(NodeAllocator & alloc, sl_type_only && src) KERBAL_NOEXCEPT
			{
			...
			void
			sl_type_only<T>::
			k_mov_ass_ua_ane(NodeAllocator & alloc, sl_type_only && src)
			{
...
#		endif
		...
		// element access
			...
			KERBAL_CONSTEXPR20
			typename
			sl_type_only<T>::const_reference
			sl_type_only<T>::
...
#		endif
		...
		// erase
			...
			k_clear_using_allocator(NodeAllocator & alloc)
				KERBAL_CONDITIONAL_NOEXCEPT(
					noexcept(
						kerbal::utility::declthis<sl_type_only>()->k_consecutive_destroy_node(
							alloc,
		...
		// operation
			...
			void
			sl_type_only<T>::
			iter_swap(iterator a, iterator b) KERBAL_NOEXCEPT
			{
			...
			void
			sl_type_only<T>::
			reverse(iterator first, iterator last) KERBAL_NOEXCEPT
			{
...
#		if KERBAL_HAS_EXCEPTIONS_SUPPORT
			...
			)
			{
				...
				node * p = node_allocator_traits::template allocate_fixed<1>(alloc);
				try {
					node_allocator_traits::construct(alloc, p, kerbal::utility::in_place_t(), kerbal::utility::forward<Args>(args)...);
				} catch (...) {
					node_allocator_traits::template deallocate_fixed<1>(alloc, p);
					throw;
				}
				return p;
			}
			...
			k_build_new_node_impl(
				...
				KERBAL_CONDITIONAL_NOEXCEPT(
					noexcept(kerbal::memory::allocator_traits<NodeAllocator>::template allocate_fixed<1>(alloc))
				)
			{
				typedef kerbal::memory::allocator_traits<NodeAllocator> node_allocator_traits;
				node * p = node_allocator_traits::template allocate_fixed<1>(alloc);
...
#			endif // KERBAL_HAS_EXCEPTIONS_SUPPORT
			...
			k_destroy_node(NodeAllocator & alloc, node_base * p_node_base)
				KERBAL_CONDITIONAL_NOEXCEPT(
					...
					noexcept(kerbal::memory::allocator_traits<NodeAllocator>::template deallocate_fixed<1>(alloc, kerbal::utility::declval<node *>()))
				)
			{
				typedef kerbal::memory::allocator_traits<NodeAllocator> node_allocator_traits;
				node * p_node = node::reinterpret_as(p_node_base);
				node_allocator_traits::destroy(alloc, p_node);
				node_allocator_traits::template deallocate_fixed<1>(alloc, p_node);
			}
