...
#	endif
		...
		static_vector<T, N> &
		static_vector<T, N>::
		operator=(const static_vector & src)
		{
...
#	endif
		...
		void
		static_vector<T, N>::
		assign(size_type new_size, const_reference val)
		{
			if (new_size > N) {
		...
		void
		static_vector<T, N>::
		k_assign_unsafe_range(InputIterator first, InputIterator last, std::input_iterator_tag)
		{
		...
		void
		static_vector<T, N>::
		k_assign_unsafe_range(ForwardIterator first, ForwardIterator last, std::forward_iterator_tag)
		{
		...
		void
		static_vector<T, N>::
		k_assign_range(InputIterator first, InputIterator last, std::input_iterator_tag)
		{
			iterator it(this->begin());
			iterator end(this->end());
			while (it != end) {
		...
		void
		static_vector<T, N>::
		k_assign_range(ForwardIterator first, ForwardIterator last, std::forward_iterator_tag)
		{
			size_type ori_size = static_cast<size_type>(this->len);
			size_type new_size = static_cast<size_type>(kerbal::iterator::distance(first, last));
...
#	if __cplusplus >= 201103L
		...
		void
		static_vector<T, N>::
		assign_unsafe(std::initializer_list<value_type> ilist)
		{
			this->assign_unsafe(ilist.begin(), ilist.end());
		}
...
#	else
		...
		void
		static_vector<T, N>::
		assign_unsafe(const kerbal::assign::assign_list<U> & ilist)
		{
...
#	endif
		...
		KERBAL_CONSTEXPR14
		typename
		static_vector<T, N>::const_reference
		static_vector<T, N>::
		...
		template <typename T, std::size_t N>
		typename
		static_vector<T, N>::equal_c_array_reference
		static_vector<T, N>::
		c_arr()
		{
			if (!full()) {
		...
		template <typename T, std::size_t N>
		typename
		static_vector<T, N>::const_equal_c_array_reference
		static_vector<T, N>::
		c_arr() const
		{
			if (!full()) {
		...
		template <typename T, std::size_t N>
		typename
		static_vector<T, N>::const_pointer
		static_vector<T, N>::
...
#	else
		...
		static_vector<T, N>::iterator
		static_vector<T, N>::
		insert(const_iterator pos, const kerbal::assign::assign_list<void> & ilist)
		{
		...
		static_vector<T, N>::iterator
		static_vector<T, N>::
		insert(const_iterator pos, const kerbal::assign::assign_list<U> & ilist)
		{
...
#	if __cplusplus >= 201103L
		...
		static_vector<T, N>::iterator
		static_vector<T, N>::
		emplace_unsafe(const_iterator pos, Args && ... args)
		{
			iterator pos_mut(pos.cast_to_mutable());
			if (pos == this->cend()) {
...
#	define FBODY(i) \
		...
		static_vector<T, N>::iterator \
		static_vector<T, N>:: \
		emplace_unsafe(const_iterator pos KERBAL_OPT_PPEXPAND_WITH_COMMA_N(LEFT_JOIN_COMMA, EMPTY, ARGS_DECL, i)) \
		{ \
 ...
 \
		...
		static_vector<T, N>::iterator \
		static_vector<T, N>:: \
		emplace(const_iterator pos KERBAL_OPT_PPEXPAND_WITH_COMMA_N(LEFT_JOIN_COMMA, EMPTY, ARGS_DECL, i)) \
		{ \
			if (static_cast<size_type>(this->len) + 1u > N) { \
				kerbal::utility::throw_this_exception_helper<std::logic_error>::throw_this_exception( \
					(const char *)"Out of storage space" \
...
#	endif
		...
		void
		static_vector<T, N>::
		k_swap_helper(static_vector & with)
		{
