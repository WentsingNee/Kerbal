...
#		endif
			...
			public:
				...
				KERBAL_CONSTEXPR20
				explicit flat_map(key_compare kc) :
					super(kc)
				{
				}
...
#		endif
				...
				KERBAL_CONSTEXPR20
				void assign(const flat_map & src)
				{
					this->ordered.assign(src.ordered);
				}

				KERBAL_CONSTEXPR20
				flat_map & operator=(const flat_map & src)
				{
					this->assign(src);
					return *this;
				}
...
#		else
				...
				template <typename U>
				flat_map & operator=(const kerbal::assign::assign_list<U> & ilist)
				{
					this->super::assign(ilist);
					return *this;
				}
