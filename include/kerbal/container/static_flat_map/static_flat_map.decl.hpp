...
#ifndef KERBAL_CONTAINER_STATIC_FLAT_MAP_STATIC_FLAT_MAP_DECL_HPP
#define KERBAL_CONTAINER_STATIC_FLAT_MAP_STATIC_FLAT_MAP_DECL_HPP

#include <kerbal/container/static_flat_map/static_flat_map.fwd.hpp>
...
#		endif
			...
			public:
				...
				KERBAL_CONSTEXPR
				explicit
				static_flat_map(key_compare kc) :
					super(kc)
				{
				}
...
#		if __cplusplus >= 201103L
				...
				static_flat_map(const static_flat_map & src) = default;
				static_flat_map(static_flat_map && src) = default;
...
#		if __cplusplus >= 201103L
				...
				KERBAL_CONSTEXPR14
				static_flat_map(std::initializer_list<value_type> ilist, key_compare kc) :
					super(ilist, kc)
				{
				}
...
#		else
				...
				template <typename U>
				static_flat_map(const kerbal::assign::assign_list<U> & ilist) :
					super(ilist)
				{
				}

				template <typename U>
				static_flat_map(const kerbal::assign::assign_list<U> & ilist, key_compare kc) :
					super(ilist, kc)
				{
				}
...
#		endif
			...
			public:
				...
				KERBAL_CONSTEXPR14
				void
				assign(const static_flat_map & src)
				{
...
#		endif
				...
				KERBAL_CONSTEXPR14
				static_flat_map &
				operator=(const static_flat_map & src)
				{
					this->assign(src);
					return *this;
				}
...
#		endif
			...
			// operation
				...
				bool
				operator==(
					const static_flat_map<K2, M2, N1, KeyCompare2> & lhs,
					const static_flat_map<K2, M2, N2, KeyCompare2> & rhs
				);
		...
		bool
		operator==(
			const static_flat_map<K, M, N1, KeyCompare> & lhs,
			const static_flat_map<K, M, N2, KeyCompare> & rhs
		)
		{
	...
	namespace algorithm
	{
		...
		void
		swap(
			kerbal::container::static_flat_map<K, M, N, KeyCompare> & a,
			kerbal::container::static_flat_map<K, M, N, KeyCompare> & b
		)
...
#endif // KERBAL_CONTAINER_STATIC_FLAT_MAP_STATIC_FLAT_MAP_DECL_HPP
