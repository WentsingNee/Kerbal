...
#ifndef KERBAL_CONTAINER_AVL_MULTISET_AVL_MULTISET_DECL_HPP
#define KERBAL_CONTAINER_AVL_MULTISET_AVL_MULTISET_DECL_HPP

#include <kerbal/container/avl_multiset/avl_multiset.fwd.hpp>
...
namespace kerbal
{
	...
	namespace container
	{
		...
		class avl_multiset : protected kerbal::container::avl_ordered<T, kerbal::container::identity_extractor<T>, KeyCompare, Allocator>
		{
			private:
...
#		endif
				...
				KERBAL_CONSTEXPR20
				avl_multiset(InputIterator first, InputIterator last);

				template <typename InputIterator>
				KERBAL_CONSTEXPR20
				avl_multiset(InputIterator first, InputIterator last, const Allocator & alloc);

				template <typename InputIterator>
				KERBAL_CONSTEXPR20
				avl_multiset(InputIterator first, InputIterator last, const KeyCompare & kc);

				template <typename InputIterator>
				KERBAL_CONSTEXPR20
				avl_multiset(InputIterator first, InputIterator last, const KeyCompare & kc, const Allocator & alloc);


#		if __cplusplus >= 201103L

				KERBAL_CONSTEXPR20
				avl_multiset(std::initializer_list<value_type> ilist);

				KERBAL_CONSTEXPR20
				avl_multiset(std::initializer_list<value_type> ilist, const Allocator & alloc);

				KERBAL_CONSTEXPR20
				avl_multiset(std::initializer_list<value_type> ilist, const KeyCompare & kc);

				KERBAL_CONSTEXPR20
				avl_multiset(std::initializer_list<value_type> ilist, const KeyCompare & kc, const Allocator & alloc);

#		else

				template <typename U>
				avl_multiset(const kerbal::assign::assign_list<U> & ilist);

				template <typename U>
				avl_multiset(const kerbal::assign::assign_list<U> & ilist, const Allocator & alloc);

				template <typename U>
				avl_multiset(const kerbal::assign::assign_list<U> & ilist, const KeyCompare & kc);

				template <typename U>
				avl_multiset(const kerbal::assign::assign_list<U> & ilist, const KeyCompare & kc, const Allocator & alloc);
...
#		endif
			...
			// assign
				...
				KERBAL_CONSTEXPR20
				avl_multiset & operator=(const avl_multiset & src);

				KERBAL_CONSTEXPR20
				void assign(const avl_multiset & src);

#		if __cplusplus >= 201103L

				KERBAL_CONSTEXPR20
				avl_multiset & operator=(avl_multiset && src);

				KERBAL_CONSTEXPR20
				void assign(avl_multiset && src);
...
#		if __cplusplus >= 201103L
				...
				KERBAL_CONSTEXPR20
				avl_multiset & operator=(std::initializer_list<value_type> ilist);
...
#		else
				...
				template <typename U>
				avl_multiset & operator=(const kerbal::assign::assign_list<U> & ilist);
...
#		endif
			...
			// operation
				...
				KERBAL_CONSTEXPR20
				void merge(avl_multiset<T, OtherKeyCompare, Allocator> & other);
...
#	if __cplusplus >= 201703L
		...
		template <typename InputIterator>
		avl_multiset(InputIterator, InputIterator)
		-> avl_multiset<typename kerbal::iterator::iterator_traits<InputIterator>::value_type>;
		...
		>
		avl_multiset(InputIterator, InputIterator, const KeyCompare &)
		-> avl_multiset<typename kerbal::iterator::iterator_traits<InputIterator>::value_type, KeyCompare>;

		template <typename InputIterator, typename Allocator,
				typename = typename kerbal::type_traits::enable_if<kerbal::memory::is_allocator<Allocator>::value>::type
		>
		avl_multiset(InputIterator, InputIterator, const Allocator &)
		-> avl_multiset<typename kerbal::iterator::iterator_traits<InputIterator>::value_type, kerbal::compare::binary_type_less<void, void>, Allocator>;

		template <typename InputIterator, typename KeyCompare, typename Allocator,
				typename = typename kerbal::type_traits::enable_if<kerbal::memory::is_allocator<Allocator>::value>::type
		>
		avl_multiset(InputIterator, InputIterator, const KeyCompare &, const Allocator &)
		-> avl_multiset<typename kerbal::iterator::iterator_traits<InputIterator>::value_type, KeyCompare, Allocator>;


		template <typename T>
		avl_multiset(std::initializer_list<T>)
		-> avl_multiset<T>;

		template <typename T, typename KeyCompare,
				typename = typename kerbal::type_traits::enable_if<!kerbal::memory::is_allocator<KeyCompare>::value>::type
		>
		avl_multiset(std::initializer_list<T>, const KeyCompare &)
		-> avl_multiset<T, KeyCompare>;

		template <typename T, typename Allocator,
				typename = typename kerbal::type_traits::enable_if<kerbal::memory::is_allocator<Allocator>::value>::type
		>
		avl_multiset(std::initializer_list<T>, const Allocator &)
		-> avl_multiset<T, kerbal::compare::binary_type_less<void, void>, Allocator>;

		template <typename T, typename KeyCompare, typename Allocator,
				typename = typename kerbal::type_traits::enable_if<!kerbal::memory::is_allocator<KeyCompare>::value>::type
		>
		avl_multiset(std::initializer_list<T>, KeyCompare, const Allocator &)
...
#	endif // if __cplusplus >= 201703L
		...
		KERBAL_CONSTEXPR20
		bool operator==(const avl_multiset<T, KeyCompare, Allocator> & lhs, const avl_multiset<T, KeyCompare, Allocator2> & rhs)
		{
	...
	namespace algorithm
	{
		...
		template <typename T, typename KeyCompare, typename Allocator>
		KERBAL_CONSTEXPR20
		void swap(kerbal::container::avl_multiset<T, KeyCompare, Allocator> & a, kerbal::container::avl_multiset<T, KeyCompare, Allocator> & b)
...
#endif // KERBAL_CONTAINER_AVL_MULTISET_AVL_MULTISET_DECL_HPP
