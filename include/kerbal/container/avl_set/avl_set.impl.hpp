...
#		endif
		...
		KERBAL_CONSTEXPR20
		typename avl_set<T, KeyCompare, Allocator>::const_iterator
		avl_set<T, KeyCompare, Allocator>::find_hint(const_iterator hint, const_reference key) const
		{
		...
		template <typename T, typename KeyCompare, typename Allocator>
		KERBAL_CONSTEXPR20
		bool avl_set<T, KeyCompare, Allocator>::contains_hint(const_iterator hint, const_reference key) const
		{
