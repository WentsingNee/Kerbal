...
#ifndef KERBAL_CONTAINER_AVL_MULTIMAP_AVL_MULTIMAP_DECL_HPP
#define KERBAL_CONTAINER_AVL_MULTIMAP_AVL_MULTIMAP_DECL_HPP

#include <kerbal/container/avl_multimap/avl_multimap.fwd.hpp>
...
#		endif
				...
				KERBAL_CONSTEXPR20
				avl_multimap(InputIterator first, InputIterator last);

				template <typename InputIterator>
				KERBAL_CONSTEXPR20
				avl_multimap(InputIterator first, InputIterator last, const Allocator & alloc);

				template <typename InputIterator>
				KERBAL_CONSTEXPR20
				avl_multimap(InputIterator first, InputIterator last, const KeyCompare & kc);

				template <typename InputIterator>
				KERBAL_CONSTEXPR20
				avl_multimap(InputIterator first, InputIterator last, const KeyCompare & kc, const Allocator & alloc);


#		if __cplusplus >= 201103L

				KERBAL_CONSTEXPR20
				avl_multimap(std::initializer_list<value_type> ilist);

				KERBAL_CONSTEXPR20
				avl_multimap(std::initializer_list<value_type> ilist, const Allocator & alloc);

				KERBAL_CONSTEXPR20
				avl_multimap(std::initializer_list<value_type> ilist, const KeyCompare & kc);

				KERBAL_CONSTEXPR20
				avl_multimap(std::initializer_list<value_type> ilist, const KeyCompare & kc, const Allocator & alloc);

#		else

				template <typename U>
				avl_multimap(const kerbal::assign::assign_list<U> & ilist);

				template <typename U>
				avl_multimap(const kerbal::assign::assign_list<U> & ilist, const Allocator & alloc);

				template <typename U>
				avl_multimap(const kerbal::assign::assign_list<U> & ilist, const KeyCompare & kc);

				template <typename U>
				avl_multimap(const kerbal::assign::assign_list<U> & ilist, const KeyCompare & kc, const Allocator & alloc);
...
#		endif
			...
			// assign
				...
				KERBAL_CONSTEXPR20
				avl_multimap & operator=(const avl_multimap & src);

				KERBAL_CONSTEXPR20
				void assign(const avl_multimap & src);

#		if __cplusplus >= 201103L

				KERBAL_CONSTEXPR20
				avl_multimap & operator=(avl_multimap && src);

				KERBAL_CONSTEXPR20
				void assign(avl_multimap && src);
...
#		if __cplusplus >= 201103L
				...
				KERBAL_CONSTEXPR20
				avl_multimap & operator=(std::initializer_list<value_type> ilist);
...
#		else
				...
				template <typename U>
				avl_multimap & operator=(const kerbal::assign::assign_list<U> & ilist);
...
#	if __cplusplus >= 201703L
		...
		template <typename InputIterator>
		avl_multimap(InputIterator, InputIterator)
		-> avl_multimap<typename avl_multimap_iter_key<InputIterator>::type, typename avl_multimap_iter_map<InputIterator>::type>;
		...
		>
		avl_multimap(InputIterator, InputIterator, const KeyCompare &)
		-> avl_multimap<typename avl_multimap_iter_key<InputIterator>::type, typename avl_multimap_iter_map<InputIterator>::type, KeyCompare>;

		template <typename InputIterator, typename Allocator,
				typename = typename kerbal::type_traits::enable_if<kerbal::memory::is_allocator<Allocator>::value>::type
		>
		avl_multimap(InputIterator, InputIterator, const Allocator &)
		-> avl_multimap<typename avl_multimap_iter_key<InputIterator>::type, typename avl_multimap_iter_map<InputIterator>::type, kerbal::compare::binary_type_less<void, void>, Allocator>;

		template <typename InputIterator, typename KeyCompare, typename Allocator,
				typename = typename kerbal::type_traits::enable_if<kerbal::memory::is_allocator<Allocator>::value>::type
		>
		avl_multimap(InputIterator, InputIterator, const KeyCompare &, const Allocator &)
		-> avl_multimap<typename avl_multimap_iter_key<InputIterator>::type, typename avl_multimap_iter_map<InputIterator>::type, KeyCompare, Allocator>;


		template <typename K, typename M>
		avl_multimap(std::initializer_list<kerbal::container::map_data<K, M> >)
		-> avl_multimap<K, M>;

		template <typename K, typename M, typename KeyCompare,
				typename = typename kerbal::type_traits::enable_if<!kerbal::memory::is_allocator<KeyCompare>::value>::type
		>
		avl_multimap(std::initializer_list<kerbal::container::map_data<K, M> >, const KeyCompare &)
		-> avl_multimap<K, M, KeyCompare>;

		template <typename K, typename M, typename Allocator,
				typename = typename kerbal::type_traits::enable_if<kerbal::memory::is_allocator<Allocator>::value>::type
		>
		avl_multimap(std::initializer_list<kerbal::container::map_data<K, M> >, const Allocator &)
		-> avl_multimap<K, M, kerbal::compare::binary_type_less<void, void>, Allocator>;

		template <typename K, typename M, typename KeyCompare, typename Allocator,
				typename = typename kerbal::type_traits::enable_if<kerbal::memory::is_allocator<Allocator>::value>::type
		>
		avl_multimap(std::initializer_list<kerbal::container::map_data<K, M> >, const KeyCompare &, const Allocator &)
...
#	endif // if __cplusplus >= 201703L
		...
		KERBAL_CONSTEXPR20
		bool operator==(const avl_multimap<K, M, KeyCompare, Allocator> & lhs, const avl_multimap<K, M, KeyCompare, Allocator2> & rhs)
		{
	...
	namespace algorithm
	{
		...
		template <typename K, typename M, typename KeyCompare, typename Allocator>
		KERBAL_CONSTEXPR20
		void swap(kerbal::container::avl_multimap<K, M, KeyCompare, Allocator> & a, kerbal::container::avl_multimap<K, M, KeyCompare, Allocator> & b)
...
#endif // KERBAL_CONTAINER_AVL_MULTIMAP_AVL_MULTIMAP_DECL_HPP
