...
#		endif
				...
				KERBAL_CONSTEXPR20
				explicit
				single_list(const Allocator & alloc)
...
#		if __cplusplus >= 201103L
				...
				single_list & operator=(single_list && src)
					KERBAL_CONDITIONAL_NOEXCEPT(
						noexcept(
							kerbal::utility::declthis<single_list>()->assign(kerbal::compatibility::move(src))
						)
					)
				;
...
#	if __cplusplus >= 201703L
		...
		single_list(std::initializer_list<T>, Alloc = Alloc()) ->
		single_list<T, Alloc>;
...
#	endif
		...
		KERBAL_CONSTEXPR20
		bool operator==(
			const single_list<T, Allocator> & lhs,
			const single_list<T, Allocator> & rhs
		)
		{
	...
	namespace algorithm
	{
		...
		KERBAL_CONSTEXPR20
		void swap(
			kerbal::container::single_list<T, Allocator> & a,
			kerbal::container::single_list<T, Allocator> & b
		)
