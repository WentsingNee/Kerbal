...
/*		template <typename Entity, typename Extract, typename Hash, typename KeyEqual,
		...
		insert_no_exists(const_reference src)
		{
			...
			if (pos_result.second) {
				iterator pos(this->_K_buckets + bucket_id, this->_K_buckets + this->_K_bucket_count, pos_result);
			...
			p->set_cached_hash_code(hash_code);
			this->_K_buckets[bucket_id]._K_hook_node_after(pos_result.first, p);
			++this->_K_size;
			iterator pos(this->_K_buckets + bucket_id, this->_K_buckets + this->_K_bucket_count, pos.cast_to_mutable());
