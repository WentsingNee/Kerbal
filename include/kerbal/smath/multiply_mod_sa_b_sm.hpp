		...
		namespace detail
		{
			...
			struct multiply_mod_sa_b_sm_impl_dispatch<ResultType, a, m, 0> // m != 0 && a % m != 0
			{
				private:
					...
					KERBAL_CONSTEXPR14
					static ResultType f_impl(ResultType b, kerbal::type_traits::type_identity<void>) KERBAL_NOEXCEPT
					{
					...
					template <typename T>
					KERBAL_CONSTEXPR
					static ResultType f_impl(ResultType b, kerbal::type_traits::type_identity<T>) KERBAL_NOEXCEPT
					{
				...
				public:
					 ...
					 */
					KERBAL_CONSTEXPR14
					static ResultType f(ResultType b) KERBAL_NOEXCEPT
					{
