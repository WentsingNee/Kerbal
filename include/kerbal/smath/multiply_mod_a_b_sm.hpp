	...
	namespace smath
	{
		...
		namespace detail
		{
			...
			struct multiply_mod_a_b_sm_impl<ResultType, m, true> // m == 0
			{
				...
				public:
					KERBAL_CONSTEXPR
					static ResultType f(ResultType a, ResultType b) KERBAL_NOEXCEPT
					{
						return a * b;
					}
			};
			...
			struct multiply_mod_a_b_sm_impl<ResultType, m, false> // m != 0
			{
				private:
					...
					KERBAL_CONSTEXPR14
					static ResultType f_impl2(ResultType a, ResultType b) KERBAL_NOEXCEPT
					{
						// a * s % m
						// a < m, s < m
					...
					KERBAL_CONSTEXPR14
					static ResultType f_impl(ResultType a, ResultType b, kerbal::type_traits::type_identity<void>) KERBAL_NOEXCEPT
					{
						return f_impl2(a, b);
					}
					...
					template <typename T>
					KERBAL_CONSTEXPR14
					static ResultType f_impl(ResultType a, ResultType b, kerbal::type_traits::type_identity<T>) KERBAL_NOEXCEPT
					{
