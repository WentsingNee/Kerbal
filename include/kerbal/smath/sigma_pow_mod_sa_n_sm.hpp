	...
	namespace smath
	{
		...
		namespace detail
		{
			...
			struct sigma_pow_mod_sa_n_sm_impl<ResultType, Base, Mod, 0> // Base == 1
			{
					...
					KERBAL_CONSTEXPR14
					static ResultType f(Unsigned n) KERBAL_NOEXCEPT
					{
						KERBAL_STATIC_ASSERT(kerbal::type_traits::is_unsigned<Unsigned>::value, "n must be unsigned");
						return kerbal::smath::multiply_mod_sa_b_sm<ResultType, 1, Mod>::f(n);
					}
			};
		...
		// a ** (n - 1) + ... + a ** 3 + a ** 2 + a ** 1 + 1
		template <typename ResultType, ResultType Base, ResultType Mod>
		struct sigma_pow_mod_sa_n_sm : detail::sigma_pow_mod_sa_n_sm_impl<ResultType, Base, Mod>
		{
			private:
