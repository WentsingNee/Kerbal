...
namespace kerbal
{
	...
	namespace algorithm
	{
		...
		namespace detail
		{
			...
			K_mismatch_with_distance(InputIterator1 first1, SentinelIterator1 last1, InputIterator2 first2, BinaryPredicate predicate, std::input_iterator_tag)
			{
				std::size_t dist = 0;
				while (first1 != last1) {
				...
				return kerbal::algorithm::mismatch_with_distance_result_t<InputIterator1, InputIterator2>(first1, first2, dist);
			}

			template <typename InputIterator1, typename SentinelIterator1, typename InputIterator2, typename BinaryPredicate>
			KERBAL_CONSTEXPR14
			mismatch_with_distance_result_t<InputIterator1, InputIterator2>
			K_mismatch_with_distance(InputIterator1 first1, SentinelIterator1 last1, InputIterator2 first2, BinaryPredicate predicate, std::random_access_iterator_tag)
			{
				...
				return kerbal::algorithm::mismatch_with_distance_result_t<InputIterator1, InputIterator2>(it, first2, kerbal::iterator::distance(first1, it));
			}
		...
		{
			return kerbal::algorithm::detail::K_mismatch_with_distance(first1, last1, first2, pred, kerbal::iterator::iterator_category(first1));
		}
