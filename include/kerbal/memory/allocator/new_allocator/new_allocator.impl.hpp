...
namespace kerbal
{
	...
	namespace memory
	{
		...
		inline
		typename new_allocator<void>::pointer
		new_allocator<void>::allocate(size_type n)
		{
			return ::operator new(n, kerbal::memory::basic_t());
		}

		inline
		void new_allocator<void>::deallocate(pointer p) KERBAL_NOEXCEPT
		{
			::operator delete(p, kerbal::memory::basic_t());
		}

		inline
		void new_allocator<void>::deallocate(pointer p, size_type /*n*/) KERBAL_NOEXCEPT
		{
			::operator delete(p, kerbal::memory::basic_t());
		}
		...
		template <typename T>
		KERBAL_CONSTEXPR20
		typename new_allocator<T>::pointer
		new_allocator<T>::allocate(size_type n)
		{
...
#	endif
		...
		template <typename T>
		KERBAL_CONSTEXPR20
		void new_allocator<T>::deallocate(pointer p, size_type n) KERBAL_NOEXCEPT
		{
