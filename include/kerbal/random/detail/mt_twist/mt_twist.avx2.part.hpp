			...
			namespace avx2
			{
				...
				void mt_twist(kerbal::compatibility::uint32_t mt[]) KERBAL_NOEXCEPT
				{
					...
					if (FIRST_STEP_REMAIN::value != 0) {
						...
						__m256i ymm_mtipm1 = _mm256_maskload_epi32(reinterpret_cast<const int*>(&mt[i + M]), ymm_maskload_mask1); // AVX2
						__m256i ymm_mtipm2 = _mm256_maskload_epi32(reinterpret_cast<const int*>(&mt[-FIRST_STEP_REMAIN::value]), ymm_maskload_mask2); // AVX2
...
#	endif
				...
				void mt_twist(kerbal::compatibility::uint64_t mt[]) KERBAL_NOEXCEPT
				{
					...
					if (FIRST_STEP_REMAIN::value != 0) {
						...
						__m256i ymm_mtipm1 = _mm256_maskload_epi64(reinterpret_cast<const long long*>(&mt[i + M]), ymm_maskload_mask1); // AVX2
						__m256i ymm_mtipm2 = _mm256_maskload_epi64(reinterpret_cast<const long long*>(&mt[-FIRST_STEP_REMAIN::value]), ymm_maskload_mask2); // AVX2
						__m256i ymm_mtipm = _mm256_or_si256(ymm_mtipm1, ymm_mtipm2); // AVX2
						ymm_mti = _mm256_xor_si256(ymm_mti, ymm_mtipm); // AVX2
						_mm256_storeu_si256(reinterpret_cast<__m256i*>(&mt[i]), ymm_mti); // AVX
						i += STEP::value;
