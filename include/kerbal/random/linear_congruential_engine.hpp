			...
			public:
				...
				result_type operator()() KERBAL_NOEXCEPT
				{
					if (m == 0) {
						...
						state_value = kerbal::smath::multiply_mod_sa_b_sm<result_type, a, m>::cacl(state_value);
						state_value = kerbal::smath::add_mod_sm<result_type, m>::cacl(state_value, c);
					}
					return state_value;
				}
				...
				void discard(unsigned long long times) KERBAL_NOEXCEPT
				{
					 ...
					 */
					result_type a_pow_times_mod_m = kerbal::smath::pow_mod_sa_n_sm<result_type, a, m>::cacl(times);
					state_value = kerbal::smath::multiply_mod_a_b_sm<ResultType, m>::cacl(a_pow_times_mod_m, state_value);
					if (c != 0) {
						if (a == 1) {
							result_type c_mul_times_mod_m = kerbal::smath::multiply_mod_sa_b_sm<result_type, c, m>::cacl(times);
							state_value = kerbal::smath::add_mod_sm<result_type, m>::cacl(state_value, c_mul_times_mod_m);
						} else {
							result_type sigma = kerbal::smath::sigma_pow_mod_sa_n_sm<result_type, a, m>::cacl(times);
							sigma = kerbal::smath::multiply_mod_sa_b_sm<result_type, c, m>::cacl(sigma);
							state_value = kerbal::smath::add_mod_sm<result_type, m>::cacl(state_value, sigma);
						}
					}
				}
