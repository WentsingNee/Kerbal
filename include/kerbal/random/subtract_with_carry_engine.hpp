...
#		endif
			...
			private:
				...
				typedef kerbal::smath::two_pow_sn<result_type, W> k_mod;
				typedef kerbal::smath::xmod<result_type, k_mod::value> xmodm;
				typedef kerbal::random::linear_congruential_engine<result_type, 40014u, 0u, 2147483563u> seed_lcg_t;

				template <result_type K, std::size_t I>
				KERBAL_CONSTEXPR14
				static
				typename kerbal::type_traits::enable_if<I == 0, result_type>::type
				k_expand_init_state(seed_lcg_t & lcg)
				{
					return xmodm::f(lcg());
				}
				...
				k_expand_init_state(seed_lcg_t & lcg)
				{
					typedef kerbal::smath::pow_mod_sa_sn_sm<result_type, std::size_t, K, I, k_mod::value> factor;
					result_type lhs = k_expand_init_state<K, I - 1>(lcg);
					result_type rhs = kerbal::smath::multiply_mod_sa_b_sm<result_type, factor::value, k_mod::value>::f(lcg());
					return kerbal::smath::add_mod_sm<result_type, k_mod::value>::f(lhs, rhs);
				}
			...
			public:
				...
				{
					seed_lcg_t lcg(seed == 0u ? DEFAULT_SEED::value : seed);
			...
			public:
				...
				{
					return kerbal::smath::two_pow_sn_minus_one<result_type, W>::value; // max == 2 ** w - 1 == m - 1
				}
