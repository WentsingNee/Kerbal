...
#include <kerbal/smath/two_pow_n_minus_one.hpp>
	...
	namespace random
	{
				...
				detail::mt_generate_copy_n_helper<UIntType, U, D, S, B, T, C, L>
		{
			...
			private:
				...
				// (2 ** W) - 1u
				typedef kerbal::smath::two_pow_n_minus_one<UIntType, W> two_pow_n_minus_one;

				KERBAL_STATIC_ASSERT(A <= two_pow_n_minus_one::value, "the following relations shall hold: A <= (2 ** W) - 1u");
				KERBAL_STATIC_ASSERT(B <= two_pow_n_minus_one::value, "the following relations shall hold: B <= (2 ** W) - 1u");
				KERBAL_STATIC_ASSERT(C <= two_pow_n_minus_one::value, "the following relations shall hold: C <= (2 ** W) - 1u");
				KERBAL_STATIC_ASSERT(D <= two_pow_n_minus_one::value, "the following relations shall hold: D <= (2 ** W) - 1u");
				KERBAL_STATIC_ASSERT(F <= two_pow_n_minus_one::value, "the following relations shall hold: F <= (2 ** W) - 1u");
...
#		endif
			...
			public:
				...
				{
					return two_pow_n_minus_one::value;
				}
