			...
			namespace avx2
			{
				...
				inline bool all_chunk(const kerbal::compatibility::uint32_t block[], std::size_t chunk_size) KERBAL_NOEXCEPT
				{
...

					{
						...
						__m256i ymm_mask = _mm256_cmpgt_epi32(_mm256_set1_epi32(static_cast<int>(chunk_size - i)), ymm_iota); // AVX2
				...
				inline bool all_chunk(const kerbal::compatibility::uint64_t block[], std::size_t chunk_size) KERBAL_NOEXCEPT
				{
...

					{
						...
						__m256i ymm_mask = _mm256_cmpgt_epi64(_mm256_set1_epi64x(static_cast<long long>(chunk_size - i)), ymm_iota); // AVX2

						__m256i ymm_block = _mm256_maskload_epi64(reinterpret_cast<const long long*>(&block[i]), ymm_mask); // AVX2
						ymm_block = _mm256_andnot_si256(ymm_block, ymm_mask); //AVX2
						if (!_mm256_testz_si256(ymm_block, ymm_ALL_ONE)) { // AVX
							return false;
						}
