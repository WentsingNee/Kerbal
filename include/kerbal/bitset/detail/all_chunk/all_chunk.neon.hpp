			...
			namespace neon
			{
				...
				inline bool all_chunk(const kerbal::compatibility::uint32_t block[], std::size_t trunk_size) KERBAL_NOEXCEPT
				{
					...
					for (; i + STEP::value <= trunk_size; i += STEP::value) {
						uint64x2_t q_block = vreinterpretq_u64_u32(vld1q_u32(&block[i]));
						if (!test_all_ones(q_block)) {
							return false;
						}
					}
				...
				inline bool all_chunk4(const kerbal::compatibility::uint32_t block[], std::size_t trunk_size) KERBAL_NOEXCEPT
				{
					...
					typedef kerbal::type_traits::integral_constant<std::size_t, 128 / 32> STEP;
					const uint32x4_t q_ALL_ONE = vdupq_n_u32(ALL_ONE::value);

					std::size_t i = 0;
					for (; i + 4 * STEP::value <= trunk_size; i += 4 * STEP::value) {
						uint32x4x4_t qx4_block = vld4q_u32(&block[i]);

						qx4_block.val[0] = vandq_u32(qx4_block.val[0], qx4_block.val[1]);
						qx4_block.val[0] = vandq_u32(qx4_block.val[0], qx4_block.val[2]);
						qx4_block.val[0] = vandq_u32(qx4_block.val[0], qx4_block.val[3]);

						if (!test_all_ones(vreinterpretq_u64_u32(qx4_block.val[0]))) {
							return false;
						}
					}
