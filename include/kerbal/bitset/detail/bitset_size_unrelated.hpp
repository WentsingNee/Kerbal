...
#				undef EACH
					...
					KERBAL_CONSTEXPR14
					static block_size_type countl_zero_chunk(const block_type block[], block_size_type block_width) KERBAL_NOEXCEPT
					{
						block_size_type i = 0;
						while (i < block_width) {
							if (block[i]) {
					...
					KERBAL_CONSTEXPR14
					static block_size_type countl_one_chunk(const block_type block[], block_size_type block_width) KERBAL_NOEXCEPT
					{
						block_size_type i = 0;
						while (i < block_width) {
							if (block[i] != ALL_ONE::value) {
