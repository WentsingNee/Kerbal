...
namespace kerbal
{
	...
	namespace assign
	{
		...
		assign_list<Tp>::assign_list()
		{
			v.k_reserve_using_allocator(this->alloc(), 20);
		}

		template <typename Tp>
		assign_list<Tp>::~assign_list() KERBAL_NOEXCEPT
		{
		...
		template <typename Tp>
		assign_list<Tp> &
		assign_list<Tp>::operator,(const_reference val)
		{
		...
		template <typename Tp>
		typename assign_list<Tp>::iterator
		assign_list<Tp>::begin()
		{
			return v.begin();
		}
		...
		template <typename Tp>
		typename assign_list<Tp>::const_iterator
		assign_list<Tp>::begin() const
		{
			return v.begin();
		}
