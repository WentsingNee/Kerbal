...
#ifndef KERBAL_HASH_IMPL_MD5_IMPL_HPP
#define KERBAL_HASH_IMPL_MD5_IMPL_HPP
...
namespace kerbal
{
	...
	namespace hash
	{
		...
		template <>
		KERBAL_CONSTEXPR14
		uint32_t MD5_context_base::blk0<LITTLE_ENDIAN>(uint32_t l[16], size_t i) KERBAL_NOEXCEPT
		{
		...
		template <>
		KERBAL_CONSTEXPR14
		uint32_t MD5_context_base::blk0<BIG_ENDIAN>(uint32_t l[16], size_t i) KERBAL_NOEXCEPT
		{
			return l[i];
		}
		...
		KERBAL_CONSTEXPR
		inline
		uint32_t MD5_context_base::blk(const uint32_t l[16], size_t i) KERBAL_NOEXCEPT
		{
		...
		KERBAL_CONSTEXPR14
		inline
		void MD5_context_base::R0(uint32_t block_l[16], uint32_t v, uint32_t & w, uint32_t x, uint32_t y, uint32_t & z, size_t i) KERBAL_NOEXCEPT
		{
			v += blk0<BYTE_ORDER>(block_l, i);
			v += kerbal::numeric::rotl(v, 5);
			v += (w & x) | (!w & y);
		...
		KERBAL_CONSTEXPR14
		inline
		void MD5_context_base::R1(uint32_t block_l[16], uint32_t v, uint32_t & w, uint32_t x, uint32_t y, uint32_t & z, size_t i) KERBAL_NOEXCEPT
		{
		...
		KERBAL_CONSTEXPR14
		inline
		void MD5_context_base::R2(uint32_t block_l[16], uint32_t v, uint32_t & w, uint32_t x, uint32_t y, uint32_t & z, size_t i) KERBAL_NOEXCEPT
		{
			z += (block_l[i] = blk(block_l, i));
			z += kerbal::numeric::rotl(v, 5);
			z += w ^ x ^ y;
		...
		KERBAL_CONSTEXPR14
		inline
		void MD5_context_base::R3(uint32_t block_l[16], uint32_t v, uint32_t & w, uint32_t x, uint32_t y, uint32_t & z, size_t i) KERBAL_NOEXCEPT
		{
			z += (block_l[i] = blk(block_l, i));
			z += kerbal::numeric::rotl(v, 5);
			z += x ^ (w | !y);
		...
		KERBAL_CONSTEXPR14
		inline
		void MD5_context_base::R4(uint32_t block_l[16], uint32_t v, uint32_t & w, uint32_t x, uint32_t y, uint32_t & z, size_t i) KERBAL_NOEXCEPT
		{
			z += (block_l[i] = blk(block_l, i));
			z += kerbal::numeric::rotl(v, 5);
			z += (w ^ x ^ y) + 0xCA62C1D6;
...
#	if __cplusplus >= 201103L
		...
		constexpr
		MD5_context_base::MD5_context_base() noexcept :
...
#	else
		...
		MD5_context_base::MD5_context_base() KERBAL_NOEXCEPT :
				a(0x67452301), b(0xefcdab89), c(0x98badcfe), d(0x10325476)
		{
...
#	endif
		...
		KERBAL_CONSTEXPR14
		inline
		void MD5_transform_overload<MD5_policy::size>::transform(const unsigned char buffer[64]) KERBAL_NOEXCEPT
		{
			uint32_t l[16] = {};
			for (int i = 0; i < 16; ++i) {
		...
		KERBAL_CONSTEXPR14
		inline
		void MD5_transform_overload<MD5_policy::fast>::transform(const unsigned char buffer[64]) KERBAL_NOEXCEPT
		{
			uint32_t l[16] = {};
			for (int i = 0; i < 16; ++i) {
				l[i] = char4tolong1(buffer + 4 * i);
		...
		KERBAL_CONSTEXPR14
		void MD5_context<Policy>::update_helper(RandomAccessIterator first, RandomAccessIterator last,
...
//		}
		...
		template <typename Policy>
		KERBAL_CONSTEXPR14
		void MD5_context<Policy>::update(const uint8_t * first, const uint8_t * last) KERBAL_NOEXCEPT
		{
		...
		KERBAL_CONSTEXPR14
		void MD5_context<Policy>::update(ForwardIterator first, ForwardIterator last) KERBAL_NOEXCEPT
		{
		...
		template <typename Policy>
		KERBAL_CONSTEXPR14
		typename MD5_context<Policy>::result
		MD5_context<Policy>::digest() KERBAL_NOEXCEPT
		{
			...
			this->update(final_count, final_count + 8); /* Should cause a MD5Transform() */
...
#endif // KERBAL_HASH_IMPL_MD5_IMPL_HPP
