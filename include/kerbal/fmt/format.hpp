	...
	namespace fmt
	{
		...
		class format_parse_context
		{
			...
			public:
				...
				format_parse_context(OutputIterator out, ForwardIterator fmt_first, ForwardIterator fmt_last):
				...
				void parse_brace(size_t arg_id, const kerbal::container::vector<formatter_and_value_pack> & pack)
				{
					if (*M_current >= '0' && *M_current <= '9') {
						...
						if (arg_id == 0) {
							...
							kerbal::fmt::format_error::throw_this_exception("the arg-ids in a format string must all be present or all be omitted");
						}
						arg_id = parse_arg_id();
					}
					if (arg_id >= pack.size()) {
						kerbal::fmt::format_error::throw_this_exception("too few arguments");
					}
					// {123.
					//     ^
					//     M_current
					if (M_current == M_last) {
						kerbal::fmt::format_error::throw_this_exception("no matched '}'");
					}
					if (*M_current == ':') {
						++M_current;
						// {123:.
						...
						kerbal::fmt::format_error::throw_this_exception("invalid format string");
					}
				}
					...
					while (M_current != M_last) {
						if (*M_current == '{') {
							...
							if (M_current == M_last || *M_current != '}') {
								kerbal::fmt::format_error::throw_this_exception("unmatched '}' in format string");
							}
							// }}
							*M_out = '}'; ++M_out; ++M_current;
						} else {
		...
		std::basic_string<typename kerbal::iterator::iterator_traits<ForwardIterator>::value_type>
		format_range(ForwardIterator fmt_first, ForwardIterator fmt_last, Args&& ... args)
		{
			typedef typename kerbal::iterator::iterator_traits<ForwardIterator>::value_type CharT;
			std::basic_string<CharT> r;
			kerbal::fmt::format_range_to(std::back_inserter(r), fmt_first, fmt_last, kerbal::utility::forward<Args>(args)...);
			return r;
		}
		...
		{
			return kerbal::fmt::format_range(fmt, fmt + std::char_traits<CharT>::length(fmt), kerbal::utility::forward<Args>(args)...);
		}

		template <typename CharT, typename ... Args>
		std::basic_string<CharT> format(const std::basic_string<CharT> & fmt, Args&& ... args)
		{
			return kerbal::fmt::format_range(fmt.c_str(), fmt.c_str() + fmt.size(), kerbal::utility::forward<Args>(args)...);
		}

		template <typename Ostream, typename CharT, typename ... Args>
		Ostream& printf(Ostream & out, const CharT * fmt, Args&& ... args)
		{
			kerbal::fmt::format_range_to(std::ostream_iterator<CharT, CharT>(out), fmt, fmt + std::char_traits<CharT>::length(fmt), kerbal::utility::forward<Args>(args)...);
			return out;
		}
		...
		{
			kerbal::fmt::format_range_to(std::ostream_iterator<CharT, CharT>(out), fmt.c_str(), fmt.c_str() + fmt.size(), kerbal::utility::forward<Args>(args)...);
			return out;
		}
