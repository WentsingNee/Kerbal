...
#include <kerbal/data_struct/array_2d.hpp>
		...
		namespace matrix
		{
			...
			class Matrix: public Array_2d<double>
			{
				...
				public:
					...
					static const Matrix load_from(std::ifstream & fin) throw(std::runtime_error);

					/**
					 *
					 * @param file_name
					 * @return
					 * @throw std::runtime_error 当文件打开失败时, 抛出此异常
					 */
					...
					/**
					 * @brief 构造一个 n 阶对角矩阵, 主对角线上的值依次为 arg0, arg1, arg2 ...
					 * @param src 主对角线上其他元素构成的参数包
					 * @return 对角矩阵
					 */
					static const Matrix diag(const Complexor<double> & src);
