...
#include <kerbal/numeric/bit/detail/countr_zero/countr_zero.gnu_builtin.part.hpp>
...
namespace kerbal
{
	...
	namespace numeric
	{
		...
		namespace detail
		{
			...
			KERBAL_CONSTEXPR
			int countr_zero_sign_handler_impl(Unsigned x, kerbal::type_traits::false_type) KERBAL_NOEXCEPT
			{
				KERBAL_STATIC_ASSERT(kerbal::type_traits::is_unsigned<Unsigned>::value, "x should be unsigned");
				return kerbal::numeric::detail::gnu_builtin::countr_zero(x);
			}
			...
			template <typename T>
			KERBAL_CONSTEXPR
			int countr_zero_sign_handler(T x) KERBAL_NOEXCEPT
			{
				return countr_zero_sign_handler_impl(x, kerbal::type_traits::is_signed<T>());
			}
		...
		template <typename T>
		KERBAL_CONSTEXPR
		int countr_zero(T x) KERBAL_NOEXCEPT
		{
			return kerbal::numeric::detail::countr_zero_sign_handler(x);
		}
