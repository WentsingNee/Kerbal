...
function(kerbal_ies_required)
    ...
    foreach (ie IN LISTS required_ies)
        if (NOT ie IN_LIST KERBAL_IE_LIST)
            message(WARNING "`${ie}` is not an instruction extension officially supported in Kerbal")
        endif ()
        ...
        if (CMAKE_CXX_COMPILER_ID MATCHES "MSVC")
            if (CMAKE_SYSTEM_PROCESSOR MATCHES "AMD64")
                ...
                elseif (ie MATCHES "^((avx2)|(avx)|(sse2)|(sse))$")
                    ...
                    continue()
                elseif (ie STREQUAL "rdrnd")
                    if (CMAKE_CXX_COMPILER_VERSION VERSION_GREATER_EQUAL "19.0.0") # msvc2015
                        message(STATUS "CMAKE_CXX_COMPILER_VERSION: ${CMAKE_CXX_COMPILER_VERSION}")
                        __kerbal_cache_result_of_supports_ie_flags(${ie} True)
                        continue()
                    endif ()
                endif ()
            endif ()
            ...
            if (ie STREQUAL "riscv-v")
                __kerbal_check_supports_ie_flags_and_cache(${ie} -march=rv64gv)
                continue()
            endif ()
...
function(kerbal_target_with_ies)
    ...
    foreach (ie IN LISTS ies)
        if (NOT DEFINED KERBAL_SUPPORT_${ie})
            list(APPEND not_defined_ies ${ie})
            continue()
        endif ()
    endforeach ()
    list(LENGTH not_defined_ies not_defined_ies_len)
    if (NOT not_defined_ies_len EQUAL 0)
        kerbal_ies_required(${not_defined_ies})
    endif ()
