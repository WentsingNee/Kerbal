...
namespace std
{
	void swap(kerbal::math::Matrix &a, kerbal::math::Matrix &b)
	{
		swap(a.row, b.row);
...
namespace kerbal
{
	namespace math
	{
		...
		void Matrix::save(const std::string &file_name) const throw (std::runtime_error)
		{
			const size_t sizeof_row = sizeof(row);
			const size_t sizeof_column = sizeof(column);
			const size_t sizeof_element = sizeof(p[0][0]);

			std::ofstream fout(&file_name[0], std::ios::out | std::ios::binary);
			//输出位宽信息
		...
		const Matrix Matrix::load_from(const std::string &file_name)
		{
			std::ifstream fin(&file_name[0], std::ios::in | std::ios::binary);
			...
			std::cout << size_of_row_in_file << std::endl;
			std::cout << size_of_column_in_file << std::endl;
			std::cout << size_of_ele_in_file << std::endl;
